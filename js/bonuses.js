/*
  Sistema de bonos y cajas diarias:
  - Inicializa los bonos disponibles y controla la lógica de reclamo.
  - Maneja las cajas diarias con premios aleatorios y actualiza saldo del usuario.
*/
  ...
  window.openDailyCase = function (day, element) {
    ...
    setTimeout(() => {
      ...
      if (day === 7) {
        ...
        if (rand < 5) prize = 100.0;
        else if (rand < 20) prize = 20.0;
        else if (rand < 50) prize = 10.0;
        else prize = 2.0 + Math.random() * 3; // Entre 2 y 5 euros
      } else {
        // Cajas Normales (Día 1-6)
        if (rand < 1) prize = 100.0;
        else if (rand < 5) prize = 10.0 + Math.random() * 10;
        else if (rand < 25) prize = 1.0 + Math.random() * 2;
        else prize = 0.1 + Math.random() * 0.4;
      }
      ...
      if (typeof Notifications !== "undefined") {
        Notifications.success(
          `¡Has abierto la caja del Día ${day} y ganado ${prize} EUR! 🎁`,
        );
      }
      ...
      element.querySelector(".case-icon").className =
        "fa-solid fa-box-open case-icon";
    }, 1000);
  };
