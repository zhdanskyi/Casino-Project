document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateProfileView() {
        ...
        // Stats
        if(elements.statWon) elements.statWon.textContent = typeof Formatter !== 'undefined' ? Formatter.currency(user.stats.totalWon, user.currency) : user.stats.totalWon + ' ' + user.currency;
        if(elements.statDeposited) elements.statDeposited.textContent = typeof Formatter !== 'undefined' ? Formatter.currency(user.stats.totalDeposited, user.currency) : user.stats.totalDeposited + ' ' + user.currency;
        if(elements.statLost) elements.statLost.textContent = typeof Formatter !== 'undefined' ? Formatter.currency(user.stats.totalLost, user.currency) : user.stats.totalLost + ' ' + user.currency;
        ...
        if (user.level >= 5) {
            ...
            if(elements.xpText) elements.xpText.textContent = `(${user.stats.totalWon.toFixed(0)} / ${nextThreshold} EUR Ganados)`;
    ...
    function renderTransactionHistory(user) {
        ...
        if (!document.querySelector('.history-table')) {
            ...
            elements.historyBody = document.getElementById('transaction-history-body');
        }
        ...
        user.transactions.forEach(tx => {
            ...
            const typeLabel = tx.type === 'deposit' ? 'Depósito' : (tx.type === 'withdraw' ? 'Retiro' : 'Juego');
    ...
    function renderBackgroundGallery(user) {
        ...
        backgrounds.forEach(bg => {
            ...
            thumb.style.border = user.bgImage === bg.url ? '2px solid var(--cyan)' : '2px solid rgba(255,255,255,0.1)';
            ...
            if (bg.url) {
                ...
                thumb.innerHTML = '<i class="fa-solid fa-ban" style="font-size: 12px; opacity: 0.5; color: #fff;"></i>';
            ...
            if (!isUnlocked) {
                ...
                thumb.addEventListener('click', () => {
                    ...
                    updateProfileView();
                    if (typeof Notifications !== 'undefined') Notifications.success("Fondo de perfil actualizado");
                });
            }
    ...
    if (elements.btnCloseBg) {
        elements.btnCloseBg.addEventListener('click', () => {
            if (elements.bgPanel) elements.bgPanel.style.transform = 'translateY(100%)';
        });
    }
        ...
        elements.avatar.addEventListener('click', () => {
            ...
            if (newUrl) {
                ...
                if(user) {
                    ...
                    if (typeof saveUserData === 'function') saveUserData(user);
                    updateProfileView();
                    if (typeof Notifications !== 'undefined') Notifications.success("Avatar actualizado");
                }
            }
        });
    }
