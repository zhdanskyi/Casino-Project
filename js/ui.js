...
const UI = {
    ...
    applyGlobalBackground: function () {
        ...
        if (user && user.bgImage) {
            ...
            if (!bgUrl.startsWith('http') && window.location.pathname.endsWith('index.html')) {
                bgUrl = bgUrl.replace('../', '');
            } else if (!bgUrl.startsWith('http') && window.location.pathname === '/') {
                bgUrl = bgUrl.replace('../', '');
            }
            ...
            document.body.style.backgroundImage = 'radial-gradient(circle at 50% 0%, rgba(20, 0, 31, 1) 0%, rgba(7, 11, 20, 1) 50%)';
        }
    },
    ...
    updateProfileDisplay: function () {
        ...
        if (user) {
            ...
            avatarElements.forEach(el => { el.innerHTML = "U"; });
        }
    },
        ...
        if (btnBuy && modal) {
            btnBuy.addEventListener('click', () => {
                ...
                setTimeout(() => {
                    ...
                    if (success) {
                        Notifications.success("¡Felicidades! Eres VIP. Neon Mines desbloqueado.");
                        modal.close();
                        if (typeof renderGames === 'function') renderGames();
                    } else {
                        Notifications.error("Saldo insuficiente para ser VIP (Requiere 1000 EUR).");
                    }
                ...
                if (tabName === 'history') {
                    ...
                    if (currentMode === 'deposit') {
                        ...
                        if (btnConfirm) {
                            ...
                            btnConfirm.style.background = 'linear-gradient(45deg, var(--cyan), #0088ff)';
                        }
                    } else {
    ...
    updateWalletModalUI: function () {
        ...
        if (walletBalance) walletBalance.textContent = Formatter.currency(user.balance, user.currency).replace(user.currency, '').trim();
