document.addEventListener('DOMContentLoaded', () => {
    ...
    function startFlight() {
        ...
        function tick() {
            ...
            // Limits
            if (planeY > 92) { planeY = 92; velocity = 0; }
    ...
    elements.container.addEventListener('touchstart', (e) => { e.preventDefault(); flap(); });
    ...
    function spawnObject() {
        ...
        gameObjects.push({ el: obj, type: type, x: window.innerWidth + 100, isIsland });
    }
    ...
    function checkCollisions() {
        ...
        for (let i = gameObjects.length - 1; i >= 0; i--) {
            ...
            if (objData.x < -300) {
                if (objData.el.parentNode) objData.el.parentNode.removeChild(objData.el);
            ...
            if (pHit.left < objRect.right && pHit.right > objRect.left &&
                pHit.top < objRect.bottom && pHit.bottom > objRect.top) {
    ...
    function winGame() {
        ...
        Notifications.success(`¡Aterrizaje Perfecto! +${Formatter.number(winAmount)} EUR`);
    ...
    // Bet controls
    const updateBet = (val) => { if(elements.betAmount) elements.betAmount.value = val.toFixed(2); };
