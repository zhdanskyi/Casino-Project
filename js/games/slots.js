/*
  ...
  - Controla tiradas, símbolos y jackpots.
  - Actualiza UI, calcula premios y anima los carretes.
*/
document.addEventListener("DOMContentLoaded", () => {
  ...
  let jackpots = { mini: 10.0, minor: 50.0, mega: 200.0, ultra: 1000.0 };

  // Anima la transición de un valor numérico en la UI, usado para jackpots.
  ...
  // Actualiza los valores de jackpot en pantalla, con opción de animación suave.
  function updateJackpotsUI(animate = false, oldJackpots = null) {
    if (!animate) {
      if (elements.jackpotMini)
        elements.jackpotMini.textContent = `${jackpots.mini.toFixed(2)} €`;
      if (elements.jackpotMinor)
        elements.jackpotMinor.textContent = `${jackpots.minor.toFixed(2)} €`;
      if (elements.jackpotMega)
        elements.jackpotMega.textContent = `${jackpots.mega.toFixed(2)} €`;
      if (elements.jackpotUltra)
        elements.jackpotUltra.textContent = `${jackpots.ultra.toFixed(2)} €`;
    } else {
      if (elements.jackpotMini && oldJackpots)
        animateValue(
          elements.jackpotMini,
          oldJackpots.mini,
          jackpots.mini,
          500,
        );
      if (elements.jackpotMinor && oldJackpots)
        animateValue(
          elements.jackpotMinor,
          oldJackpots.minor,
          jackpots.minor,
          500,
        );
      if (elements.jackpotMega && oldJackpots)
        animateValue(
          elements.jackpotMega,
          oldJackpots.mega,
          jackpots.mega,
          500,
        );
      if (elements.jackpotUltra && oldJackpots)
        animateValue(
          elements.jackpotUltra,
          oldJackpots.ultra,
          jackpots.ultra,
          500,
        );
  ...
  // Ejecuta una tirada: valida apuesta, descuenta saldo, gira los carretes y determina el resultado.
  async function spin() {
    ...
    if (results[0] === results[1] && results[1] === results[2]) {
      ...
      if (sym === "👑") {
        win += jackpots.ultra;
        jackpots.ultra = 1000.0;
        isJackpot = true;
        winningPlate = "plateUltra";
      } else if (sym === "💎") {
        win += jackpots.mega;
        jackpots.mega = 200.0;
        isJackpot = true;
        winningPlate = "plateMega";
      } else if (sym === "7️⃣") {
        win += jackpots.minor;
        jackpots.minor = 50.0;
        isJackpot = true;
        winningPlate = "plateMinor";
      } else if (sym === "🍉") {
        win += jackpots.mini;
        jackpots.mini = 10.0;
        isJackpot = true;
        winningPlate = "plateMini";
      }
