/*
  Rocket game logic:
  - Simula un vuelo de cohete con multiplicador creciente.
  - El jugador puede retirar antes de que el cohete explote para ganar.
*/
document.addEventListener("DOMContentLoaded", () => {
  ...
  function generateRealisticCrash() {
    const r = Math.random();
    if (r < 0.05) return 1.0 + Math.random() * 0.5; // 5% chance to crash before 1.5x
    if (r < 0.2) return 1.5 + Math.random() * 0.5; // 15% chance to crash between 1.5x and 2.0x
    if (r < 0.85) return 2.0 + Math.random() * 8.0; // 65% chance to crash between 2.0x and 10.0x
    return 10.0 + Math.random() * 40.0; // 15% chance to crash beyond 10.0x (up to 50x)
  }

  // Inicia el juego de cohete: valida sueño, descuenta la apuesta y arranca la suba del multiplicador.
  function startGame() {
    ...
    elements.rocket.style.filter =
      "drop-shadow(0 0 10px var(--cyan)) blur(0px)";
    ...
    function tick() {
      ...
      if (currentMultiplier > 2.0) {
        ...
        document.querySelector(".rocket-view").style.transform =
  ...
  function cashout() {
    ...
    if (typeof Formatter !== "undefined") {
      Notifications.success(
        `¡Retirado a ${currentMultiplier.toFixed(2)}x! +${Formatter.number(winAmount)} EUR`,
      );
    }
  ...
  // Maneja el choque del cohete: detiene la animación, muestra explosión y restablece los controles.
  function crashGame() {
    gameState = "CRASHED";
    ...
    btnHalf.addEventListener("click", () => {
      if (elements.betAmount)
        elements.betAmount.value = (
          parseFloat(elements.betAmount.value) / 2
        ).toFixed(2);
    });
  }
    ...
    btnMax.addEventListener("click", () => {
      ...
      if (user && elements.betAmount) {
        ...
        if (typeof Notifications !== "undefined")
          Notifications.error("Debes iniciar sesión para apostar.");
      }
    });
  }
});
